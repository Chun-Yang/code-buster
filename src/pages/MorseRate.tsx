...
export default function MorseRate() {
  const [encodeRates, setEncodeRates] = useState(() =>
    readFluencyRates("encode"),
  );
  const [decodeRates, setDecodeRates] = useState(() =>
    readFluencyRates("decode"),
  );
  ...
  return (
    <div>
      ...
      <h1 style={{ textAlign: "center" }}>Fluency Score</h1>
            ...
            <tbody>
              {LETTERS.map((letter) => (
                <tr key={letter}>
                  <td>{letter}</td>
                  <td>
                    {encodeRates[letter] !== null && (
                      <span
                        className={`score-badge ${scoreClass(encodeRates[letter])}`}
                      >
                        {formatRate(encodeRates[letter])}
                      </span>
                    )}
                  </td>
                </tr>
            ...
            <tbody>
              {LETTERS.map((letter) => (
                <tr key={letter}>
                  ...
                  <td>
                    {decodeRates[letter] !== null && (
                      <span
                        className={`score-badge ${scoreClass(decodeRates[letter])}`}
                      >
                        {formatRate(decodeRates[letter])}
                      </span>
                    )}
                  </td>
                </tr>
