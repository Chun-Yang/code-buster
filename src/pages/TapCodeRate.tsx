...
import {
  ...
  resetTapCodeFluencyRates,
} from '../tapcode'

export default function TapCodeRate() {
  const [rates, setRates] = useState(() => readTapCodeFluencyRates())
  const [showConfirm, setShowConfirm] = useState(false)

  function handleReset() {
    resetTapCodeFluencyRates()
    setRates(readTapCodeFluencyRates())
  ...
  return (
    <div>
      ...
      <h1 style={{ textAlign: 'center' }}>Tap Code Fluency Score</h1>
      ...
      <table className="rate-table">
        ...
        <tbody>
          {LETTERS.map((letter) => (
            <tr key={letter}>
              <td>{letter}</td>
              <td className="rate-morse">{TAP_CODE[letter]}</td>
              <td>
                {rates[letter] !== null && (
